...
| `GET /:slug[?status=<s>&limit=<n>&cursor=<pos>&q=<text>]` | Fifo detail. `?status=` filters items (`todo` \| `lock` \| `done` \| `fail` \| `skip`; default `todo`). Order: oldest first for `todo`/`lock`, newest first for `done`/`fail`/`skip`. **`limit`**: page size — positive integer, default and max **100**. **`cursor`**: keyset continuation — omit on the first page; when `has_more`, pass the **`position`** of the **last** item in `items[]`. Next page loads `position > cursor` (`todo`/`lock`) or `position < cursor` (`done`/`fail`/`skip`). **`q`**: optional case-insensitive substring filter on raw item `data` (SQL `LIKE`). Response includes **`has_more`** and `max_retries`. Content-negotiated HTML / JSON / YAML. |
